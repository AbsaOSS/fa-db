...
trait QueryStatusHandling {
  protected def checkStatus[A](statusWithData: FunctionStatusWithData[A]): Either[StatusException, A]
}

trait StandardQueryStatusHandling extends QueryStatusHandling {
  override protected def checkStatus[A](statusWithData: FunctionStatusWithData[A]): Either[StatusException, A] = {

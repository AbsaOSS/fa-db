...
class ByFirstRowStatusAggregatorUnitTests extends AnyFunSuiteLike {
  ...
  test("aggregate should return Empty Seq in Right for an empty Sequence") {
    ...
    val actualAggData = aggregateByFirstRowUnderTest.aggregate(testData)
  ...
  test("aggregate should return data only, when there are multiple error status codes on non-first row, along with data") {
    val testData = Seq(
      Right(Row(FunctionStatus(10, "Ok"), ("FirstName1", "SecondName1"))),
      Left(DataNotFoundException(FunctionStatus(42, "Data not found"))),
      Right(Row(FunctionStatus(10, "Ok"), ("FirstName2", "SecondName2"))),

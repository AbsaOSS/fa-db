...
class DoobiePgEngine(val transactor: Transactor[IO]) extends DBEngine[IO] {
  ...
  private def run_[R](query: QueryType[R])(implicit readR: Read[R], getR: Get[R]): IO[Seq[R]] = {
    query.fragment.query[R].to[Seq].transact(transactor)
  }

  override protected def run[R](query: DoobieQuery[R]): IO[Seq[R]] =
    run_(query)(query.readR, query.getR)
}

...
class SlickPgEngine(val db: Database) extends DBEngine {
  ...
  override protected def run[QR >: PositionedResult, R](query: SlickQuery, converter: QR => R): Future[Seq[R]] = {
    ...
    val getResult: GetResult[R] = GetResult{ r =>
      val pr: PositionedResult = r
      converter(pr)
    }
    val slickAction = query.sql.as[R](getResult)
    db.run(slickAction)
  }
